...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('partner-management.create') }}">Add new partner</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('partner-management.search') }}">
         {{ csrf_field() }}
         @component('layouts.search', ['title' => 'Search'])
          @component('layouts.two-cols-search-row', ['items' => ['partner_id', 'name'],
          'oldVals' => [isset($searchingVals) ? $searchingVals['partner_id'] : '', isset($searchingVals) ? $searchingVals['name'] : '']])
          @endcomponent
          </br>
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($partners as $partner)
                <tr role="row" class="odd">
                    <td class="hidden-xs">{{ $partner->partner_id }}</td>
                    <td class="hidden-xs">{{ $partner->password }}</td>
                  <td class="sorting_1">{{ $partner->name }}</td>
                    <td class="hidden-xs">{{ $partner->cnic_number }}</td>
                    <td class="hidden-xs">{{ $partner->contact_number }}</td>
                    <td class="hidden-xs">{{ $partner->email }}</td>
                    <td class="hidden-xs">{{ $partner->rating }}</td>

                  <td>
                    <form class="row" method="POST" action="{{ route('partner-management.destroy', ['partner_id' => $partner->partner_id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('partner-management.edit', ['$partner_id' => $partner->partner_id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
                        @if (['partner_id' => $partner->partner_id])
      ...
      <div class="row">
        <div class="col-sm-4">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($partners)}} of {{count($partners)}} entries</div>
        </div>
        <div class="col-sm-7">
